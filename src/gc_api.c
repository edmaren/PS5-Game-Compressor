...
#define GC_BASE "/data/ps5-image-stack/GameCompressor"
...
#define GC_REPAIR_WAIT_SECONDS 90
#define GC_REPAIR_WAIT_STEP_SECONDS 5
#define GC_REMOUNT_WAIT_SECONDS 150
#define GC_REMOUNT_WAIT_STEP_SECONDS 3
#define GC_REPAIR_REMOUNT_PREFIX ".__gc_remount_"
...
static uint64_t
source_size_bytes(const char *path, gc_source_kind_t kind) {
  struct stat st;
...
static int
repair_remount_original_name_for_temp(const char *name,
                                      char *out, size_t out_size) {
  const char *marker;
  char title_id[64];
  size_t title_len;
  size_t prefix_len = strlen(GC_REPAIR_REMOUNT_PREFIX);
  size_t name_len = strlen(name ? name : "");
  size_t suffix_len = strlen(".ffpfsc");

  if(!name || !out || out_size == 0 || !ends_with_ci(name, ".ffpfsc")) {
    return -1;
  }
  marker = strstr(name, GC_REPAIR_REMOUNT_PREFIX);
  if(!marker) return -1;
  title_len = (size_t)(marker - name);
  if(title_len != 9 || title_len >= sizeof(title_id)) return -1;
  if(name_len <= title_len + prefix_len + suffix_len) return -1;
  memcpy(title_id, name, title_len);
  title_id[title_len] = 0;
  if(!valid_title_id(title_id)) return -1;
  if(snprintf(out, out_size, "%s.ffpfsc", title_id) >= (int)out_size) {
    return -1;
  }
  return 0;
}
...
static int
cleanup_repair_remount_temps_in_root(const char *root) {
  DIR *d = opendir(root);
  struct dirent *ent;
  int restored = 0;
  ...
  while((ent = readdir(d))) {
    ...
    if(repair_remount_original_name_for_temp(ent->d_name, original_name,
                                            sizeof(original_name)) != 0) {
      continue;
    }
    join_path(temp_path, sizeof(temp_path), root, ent->d_name);
    if(stat(temp_path, &st) != 0 || !S_ISREG(st.st_mode)) continue;
    join_path(original_path, sizeof(original_path), root, original_name);

    if(stat(original_path, &st) == 0) {
      gc_log("repair remount cleanup kept temp=%s original exists=%s",
             temp_path, original_path);
      continue;
    }
    if(errno != ENOENT) {
      gc_log("repair remount cleanup skipped temp=%s original=%s err=%s",
             temp_path, original_path, strerror(errno));
      continue;
    }
    if(rename(temp_path, original_path) != 0) {
      gc_log("repair remount cleanup restore failed temp=%s original=%s err=%s",
             temp_path, original_path, strerror(errno));
      continue;
    }
    restored++;
    gc_log("repair remount cleanup restored temp=%s original=%s",
           temp_path, original_path);
  }
  closedir(d);
  return restored;
...
static void
cleanup_repair_remount_temps_on_startup(void) {
  int restored = 0;

  for(int i = 0; GC_SHADOW_SOURCE_ROOTS[i]; i++) {
    restored += cleanup_repair_remount_temps_in_root(GC_SHADOW_SOURCE_ROOTS[i]);
  }
  if(restored > 0) {
    char err[256] = {0};
    if(gc_shadowmount_request_scan(err, sizeof(err)) != 0) {
      gc_log("repair remount cleanup scan request failed restored=%d err=%s",
             restored, err[0] ? err : "unknown");
    } else {
      gc_log("repair remount cleanup requested scan restored=%d", restored);
    }
  }
}
...
load_validation_state(gc_game_t *g) {
  ...
  uint64_t old_mtime = json_find_u64_value(json, "sourceMtime", 0);
  int marker_matches_path = strcmp(image_path, g->source_path) == 0 &&
      stat(g->source_path, &st) == 0 &&
  ...
  if(!marker_matches_path || (uint64_t)st.st_mtime != old_mtime) {
    snprintf(g->validation_status, sizeof(g->validation_status), "%s",
             "changed");
    g->validation = GC_VALIDATION_CHANGED;
  } else {
    snprintf(g->validation_status, sizeof(g->validation_status), "%s",
             "validated");
...
write_validation_marker_ex(const char *title_id, const char *image_path,
  ...
  int ok = json_append(&b, "{\n  \"titleId\":") == 0 &&
      ...
      json_appendf(&b,
                   ...
                   (unsigned long long)(info ? info->hash_mismatched_blocks : 0),
...
wait_for_shadowmount_links(const char *title_id,
  ...
  while(1) {
    ...
    if(!scan_requested || time(NULL) + GC_REMOUNT_WAIT_STEP_SECONDS < deadline) {
      scan_err[0] = 0;
    ...
    if(time(NULL) >= deadline || job_cancelled()) {
...
static void
detect_game_source(gc_game_t *g) {
  ...
  g->source_size = source_size_bytes(g->source_path, g->source_kind);
...
find_game_by_title(const char *title_id, gc_game_t *out) {
  ...
  size_t count = 0;
  discover_games(games, GC_MAX_GAMES, &count);
...
append_history_log(const gc_operation_t *op) {
  ...
  if(json_append(&b, "{\"id\":") == 0 &&
     ...
     json_appendf(&b,
                  ...
                  (unsigned long long)op->bad_blocks_found) == 0) {
...
static int
recovery_event_slot(gc_history_recovery_event_t *events, size_t count,
                    const char *title_id) {
  for(size_t i = 0; i < count; i++) {
    if(!strcmp(events[i].title_id, title_id)) return (int)i;
  }
  return -1;
}
...
recovery_append_phase_history(const gc_history_recovery_event_t *ev,
  ...
  memset(&op, 0, sizeof(op));
  op.used = 1;
  op.action = !strcmp(ev->action, "compress") ? GC_ACTION_COMPRESS :
      GC_ACTION_VALIDATE_REPAIR;
  op.status = GC_OP_RUNNING;
...
recovery_enqueue_validate_repair(const gc_history_recovery_event_t *ev) {
  ...
  start_next_locked();
  pthread_mutex_unlock(&g_gc_lock);
  gc_log("recovery enqueued validate-repair title=%s path=%s",
...
static int
build_repair_remount_temp_path(const char *path, const char *title_id,
                               char *out, size_t out_size) {
  char parent[1024];
  const char *name = base_name(path);
  if(!path || !title_id || !valid_title_id(title_id) ||
     !ends_with_ci(path, ".ffpfsc") ||
  ...
  int n = snprintf(out, out_size, "%s/%.*s%s%ld_%u.ffpfsc",
                   parent, (int)stem_len, name, GC_REPAIR_REMOUNT_PREFIX,
                   (long)time(NULL), (unsigned)getpid());
  return n < 0 || (size_t)n >= out_size ? -1 : 0;
}

static int
wait_for_repaired_shadowmount(const char *title_id, const char *path,
                              const pfs_repair_info_t *info,
                              const char *current,
                              char *err, size_t err_size) {
  char expected_image[1024];
  char expected_mount[1024];
  ...
  if(expected_compressed_shadow_paths(path, info->nested_name,
    ...
    snprintf(err, err_size, "%s", "could not derive repair remount path");
    return -1;
  }
  job_set_phase("mounting", 0, 0, current ? current : "Waiting for remount");
  if(gc_shadowmount_request_scan(scan_err, sizeof(scan_err)) != 0) {
    gc_log("repair remount scan request failed title=%s err=%s",
           title_id ? title_id : "", scan_err[0] ? scan_err : "unknown");
  }
...
static int
repair_restore_original_path(const char *title_id, const char *temp_path,
                             const char *original_path,
                             const pfs_repair_info_t *info,
                             char *err, size_t err_size) {
  if(rename(temp_path, original_path) != 0) {
    snprintf(err, err_size, "restore repaired image name: %s",
             strerror(errno));
    gc_log("repair remount restore failed title=%s temp=%s original=%s err=%s",
           title_id ? title_id : "", temp_path ? temp_path : "",
           original_path ? original_path : "", err && err[0] ? err : "");
    return -1;
  }
  gc_log("repair remount restore title=%s temp=%s original=%s",
         title_id ? title_id : "", temp_path, original_path);
  if(wait_for_repaired_shadowmount(title_id, original_path, info,
...
repair_force_path_bounce_remount(const char *title_id,
  ...
  if(!info || info->repaired_blocks == 0 ||
     info->repair_mode != PFS_REPAIR_MODE_COPY_REPLACE) {
    return post_repair_smoke_verify(title_id, original_path, info, 0,
                                    err, err_size);
  }
  if(build_repair_remount_temp_path(original_path, title_id, temp_path,
                                    sizeof(temp_path)) != 0) {
    snprintf(err, err_size, "%s", "could not build repair remount temp path");
    return -1;
  }
  if(rename(original_path, temp_path) != 0) {
    snprintf(err, err_size, "rename repaired image for remount: %s",
             strerror(errno));
    return -1;
  }
  bounced = 1;
  gc_log("repair remount bounce title=%s original=%s temp=%s",
         title_id ? title_id : "", original_path, temp_path);

  if(wait_for_repaired_shadowmount(title_id, temp_path, info,
                                   "Waiting for temporary repair remount",
                                   err, err_size) != 0) {
    goto fail_restore;
  }
  if(post_repair_smoke_verify(title_id, temp_path, info, 1,
                              err, err_size) != 0) {
    goto fail_restore;
  }
  if(repair_restore_original_path(title_id, temp_path, original_path, info,
                                  err, err_size) != 0) {
    bounced = 0;
    return -1;
  }
...
fail_restore:
  if(bounced) {
    if(repair_restore_original_path(title_id, temp_path, original_path, info,
      ...
      gc_log("repair remount restore after failure failed title=%s err=%s",
             title_id ? title_id : "", restore_err);
    }
  }
  return -1;
}
...
run_move_op(gc_operation_t *op) {
  ...
  job_set_phase("copying", 0, 0, "Moving game");
...
run_compress_op(gc_operation_t *op) {
  ...
  gc_log("compress start op=%s title=%s format=%s policy=%s", op->id,
  ...
  gc_log("compress source title=%s kind=%s size=%llu free=%llu required=%llu path=%s output=%s",
  ...
  if(gc_shadowmount_write_pfsc_hints(game.output_path, planned_nested_name,
                                     planned_nested_type, err,
                                     sizeof(err)) != 0) {
  ...
  gc_log("compress shadowmount hints title=%s outer=%s nested=%s type=%d",
         op->title_id, game.output_path, planned_nested_name,
         planned_nested_type);

  gc_checkpoint("compress writing ffpfsc");
  if(pfs_compress_source_to_ffpfsc_opts(game.source_path, 0,
  ...
  char expected_image_link[1024];
  char expected_mount_link[1024];
  gc_checkpoint("compress wait remount");
  job_set_phase("mounting", 0, 0, "Waiting for remount");
  if(expected_compressed_shadow_paths(info.output_path, info.nested_name,
                                      info.nested_type,
                                      expected_image_link,
                                      sizeof(expected_image_link),
                                      expected_mount_link,
                                      sizeof(expected_mount_link)) != 0) {
    snprintf(op->error, sizeof(op->error), "%s",
             "could not derive ShadowMount remount path");
    gc_log("compress remount failed title=%s err=%s", op->title_id,
           op->error);
    return -1;
  }
  if(wait_for_shadowmount_links(op->title_id, expected_mount_link,
                                expected_image_link, err, sizeof(err)) != 0) {
    snprintf(op->error, sizeof(op->error), "%s",
             err[0] ? err : "ShadowMountPlus remount failed");
    gc_log("compress remount failed title=%s err=%s", op->title_id,
           op->error);
  ...
  uint64_t final_compressed_size =
      source_size_bytes(info.output_path, GC_SOURCE_COMPRESSED);
...
run_uncompress_op(gc_operation_t *op) {
  ...
  gc_checkpoint("uncompress wait remount");
  job_set_phase("mounting", 0, 0, "Waiting for remount");
  if(wait_for_shadowmount_links(op->title_id, info.output_path, "",
                                err, sizeof(err)) != 0) {
    snprintf(op->error, sizeof(op->error), "%s",
             err[0] ? err : "ShadowMountPlus remount failed");
...
preflight_action(const gc_game_t *game, gc_action_t action, int confirm_stream,
  ...
  if(game->can_stream_delete && confirm_stream) {
...
enqueue_action(const http_request_t *req, gc_action_t action) {
  ...
  if(find_game_by_title(title_id, &game) != 0) {
  ...
  int p = preflight_action(&game, action, confirm_stream, delete_policy,
  ...
  pthread_mutex_lock(&g_gc_lock);
  if(pending_op_for_title_locked(title_id)) {
    pthread_mutex_unlock(&g_gc_lock);
    return serve_error(req, 409, "action already pending for this game");
  }
  gc_operation_t *op = alloc_op_locked();
  if(!op) {
    pthread_mutex_unlock(&g_gc_lock);
...
refresh_mount_request(const http_request_t *req) {
  ...
  if(gc_shadowmount_write_pfsc_hints(game.source_path, dec.nested_name,
                                     dec.nested_type, err,
                                     sizeof(err)) != 0) {
    return serve_error(req, 500,
                       err[0] ? err : "could not write ShadowMount hints");
  }
  if(gc_shadowmount_request_scan(err, sizeof(err)) != 0) {
    return serve_error(req, 500,
                       err[0] ? err : "could not request ShadowMount scan");
  }
  if(expected_compressed_shadow_paths(game.source_path, dec.nested_name,
                                      dec.nested_type, expected_image,
                                      sizeof(expected_image), expected_mount,
                                      sizeof(expected_mount)) != 0) {
    return serve_error(req, 500, "could not derive mount paths");
  }

  gc_log("refresh mount title=%s source=%s nested=%s type=%d mount=%s image=%s",
         title_id, game.source_path, dec.nested_name, dec.nested_type,
         expected_mount, expected_image);
  if(json_append(&b, "{\"ok\":true,\"titleId\":") != 0 ||
     json_string(&b, title_id) != 0 ||
     json_append(&b, ",\"sourcePath\":") != 0 ||
     json_string(&b, game.source_path) != 0 ||
     json_append(&b, ",\"nestedName\":") != 0 ||
     json_string(&b, dec.nested_name) != 0 ||
     json_appendf(&b, ",\"nestedType\":%d,\"expectedImage\":",
                  dec.nested_type) != 0 ||
     json_string(&b, expected_image) != 0 ||
     json_append(&b, ",\"expectedMount\":") != 0 ||
     json_string(&b, expected_mount) != 0 ||
     json_append(&b, "}") != 0) {
    free(b.data);
    return -1;
  }
...
operation_fallback_game(const gc_operation_t *op, gc_game_t *game) {
  ...
  snprintf(game->output_path, sizeof(game->output_path), "%s",
           op->output_path);
  game->source_size = source_size_bytes(game->source_path, game->source_kind);
  game->required_bytes = game->source_size;
  if(game->output_path[0]) {
    (void)free_bytes_for_output(game->output_path, &game->free_bytes);
  }
...
job_request(const http_request_t *req) {
  ...
  if(json_append(&b, "{\"ok\":true,\"busy\":") != 0 ||
     ...
     json_appendf(&b,
                  ...
                  "\"startedAt\":%ld,"
                  "\"elapsedSeconds\":%ld}",
                  total, copied, phase_step, phase_count, raw_blocks,
                  ...
                  (long)started_at, elapsed_seconds) != 0) {
    free(b.data);
    return -1;
  }
  return serve_owned(req, 200, b.data, b.len);
...
gc_api_recover_on_startup(void) {
  cleanup_repair_remount_temps_on_startup();

  FILE *f = fopen(GC_HISTORY_LOG, "r");
  if(!f) return;

  gc_history_recovery_event_t latest[GC_MAX_GAMES];
  size_t count = 0;
  char line[4096];
  while(fgets(line, sizeof(line), f)) {
    ...
    int slot = recovery_event_slot(latest, count, ev.title_id);
    if(slot < 0) {
      if(count >= GC_MAX_GAMES) continue;
      slot = (int)count++;
    }
    latest[slot] = ev;
  }
  ...
  for(size_t i = 0; i < count; i++) {
    if(strcmp(latest[i].status, "running") ||
       !history_phase_is_recoverable(latest[i].phase)) {
      continue;
    }
    gc_log("recovery processing title=%s action=%s phase=%s output=%s",
           latest[i].title_id, latest[i].action, latest[i].phase,
           latest[i].output_path);
    (void)recovery_process_history_event(&latest[i]);
  }
}
