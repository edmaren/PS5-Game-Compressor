...
Compared local `AMPRSupport` against `main` / `v0.9.9`
(`74a1a89`).
...
- Added APR Emu support for compressed titles. Game Compressor now performs the
  same `build_ampr_index.py` index-building work directly on the PS5 before
  compressing any title that uses APR Emu, generating a fresh
  `ampr_emu.index` during the pre-compress scan.
- Added native `ampr_emu.index` generation to safe, stream, PFS, and exFAT
  folder compression paths.
- Added an `APR indexed` UI tag for outputs where Game Compressor generated the
  APR/AMPR index, including game rows, selected-game details, and operation
  history.
...
- Non-APR titles are unchanged: Game Compressor skips APR index generation
  unless the title already has APR Emu markers or an `ampr_emu.index`.
